...
void test(TestDistribution* dist, TheoreticMoments&& theoretic) {
    ...
    ASSERT_NEAR(empirical.mean(samples), theoretic.mean(*dist), 1e-2);
    ASSERT_NEAR(empirical.variance(samples), theoretic.variance(*dist), 1e-2);
    ASSERT_NEAR(empirical.skewness(samples), theoretic.skewness(*dist), 1e-2);
    ASSERT_NEAR(empirical.kurtosis(samples), theoretic.kurtosis(*dist), 1e-2);
}

TEST(disttest, StatisticalMomentsCorrectness) {
    RandomNumberGenerator<std::mt19937> rng{ std::mt19937(detail::seed) };
